...
采用平台字幕。只有完整生成的结构化字幕才会写入用户缓存，临时视频和音频仍会在任务
结束后删除。当前版本不能从进行到一半的 OCR 或 Whisper 内部继续，但可以复用上一次
已经完成的识别结果，并重新生成全部输出文件。

缓存保存在 `~/Library/Caches/YouTubeText/transcripts/`，不会自动过期；目录和文件分别
限制为当前用户可读的 `0700`/`0600` 权限。匿名任务与不同内容的 Cookie 文件使用相互
